...
$db->exec('CREATE TABLE cubrid_test(idx int NOT NULL PRIMARY KEY, txt VARCHAR(20))');
$db->exec('INSERT INTO cubrid_test VALUES(0, \'String0\')');
$db->exec('INSERT INTO cubrid_test VALUES(1, \'String1\')');
$db->exec('INSERT INTO cubrid_test VALUES(2, \'String2\')');

$stmt1 = $db->prepare('SELECT COUNT(idx) FROM cubrid_test');
$stmt2 = $db->prepare('SELECT idx, txt FROM cubrid_test ORDER by idx');
...
$stmt3 = $db->prepare('SELECT txt FROM cubrid_test WHERE idx=:inp'); 
$stmt3->bindParam('inp', $idx); /* by foreign name */

$stmt4 = $db->prepare('SELECT idx FROM cubrid_test WHERE txt=:txt'); 
