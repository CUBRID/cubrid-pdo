...
static int pdo_cubrid_set_attribute(pdo_dbh_t *dbh, long attr, zval *val TSRMLS_DC)
{
	...
	case PDO_ATTR_AUTOCOMMIT:
		...
		if (dbh->auto_commit ^ Z_LVAL_P(val)) {
			...
			if ((cubrid_retval = cci_set_autocommit(H->conn_handle, Z_LVAL_P(val))) < 0) {
				pdo_cubrid_error(dbh, cubrid_retval, &error, NULL);
				return 0;	
			}

			H->auto_commit = Z_LVAL_P(val);
    ...
    for (i = 0; i < column_count; i++) {
		...
		if (CCI_IS_SET_TYPE(column_type) || CCI_IS_MULTISET_TYPE(column_type) || 
			...
			if (null_indicator < 0) {
				...
				if (type & CUBRID_NUM) {
					add_index_stringl(arg, i, res_buf, null_indicator);
				} 
				
				if (type & CUBRID_ASSOC) {
					add_assoc_stringl(arg, column_name, res_buf, null_indicator);
				}
			}
		}
    }
...
static int cubrid_add_assoc_array(zval *arg, char *key, T_CCI_SET in_set TSRMLS_DC)
{
    ...
    if ((cubrid_retval = zend_hash_update(HASH_OF(arg), key, (void *) &tmp_zval)) == FAILURE) {
		cubrid_array_destroy(HASH_OF(&tmp_zval) ZEND_FILE_LINE_CC);
		return CUBRID_ER_PHP;
    }
