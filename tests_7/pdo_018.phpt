...
$db->exec('CREATE TABLE cubrid_test(id int NOT NULL PRIMARY KEY, classtype int, val VARCHAR(255))');
...
echo "===INSERT===\n";
$stmt = $db->prepare('INSERT INTO cubrid_test VALUES(:id, :classtype, :val)');
...
echo "===DATA===\n";
$res = $db->query('SELECT cubrid_test.val FROM cubrid_test')->fetchAll(PDO::FETCH_COLUMN);
...
try
{
	$db->query('SELECT classtypes.name AS name, cubrid_test.val AS val FROM cubrid_test LEFT JOIN classtypes ON cubrid_test.classtype=classtypes.id')->fetchAll(PDO::FETCH_CLASS|PDO::FETCH_CLASSTYPE|PDO::FETCH_SERIALIZE, 'TestLeaf', array());
}
...
echo "===COUNT===\n";
var_dump($db->query('SELECT COUNT(*) FROM cubrid_test LEFT JOIN classtypes ON cubrid_test.classtype=classtypes.id WHERE (classtypes.id IS NULL OR classtypes.id > 0)')->fetchColumn());

echo "===DATABASE===\n";
$stmt = $db->prepare('SELECT classtypes.name AS name, cubrid_test.val AS val FROM cubrid_test LEFT JOIN classtypes ON cubrid_test.classtype=classtypes.id WHERE (classtypes.id IS NULL OR classtypes.id > 0)');
