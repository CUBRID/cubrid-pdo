...
$db->exec('CREATE TABLE cubrid_test(id INT NOT NULL PRIMARY KEY, val VARCHAR(10))'.$suf);
...
function countRows($action) {
    global $db;
	$select = $db->prepare('SELECT COUNT(*) FROM cubrid_test');
	$select->execute();
...
$delete = $db->prepare('DELETE FROM cubrid_test');
