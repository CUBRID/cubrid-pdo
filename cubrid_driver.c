		...
		char **quoted, int *quotedlen, enum pdo_param_type paramtype TSRMLS_DC)
{
	...
	*quoted = (char *) emalloc(2 * unquotedlen + 16);
...
{
	pdo_cubrid_db_handle *H = (pdo_cubrid_db_handle *)dbh->driver_data;
	int cubrid_retval = 0, lock_timeout_value = -1;
	T_CCI_ERROR error;
	...
	case PDO_CUBRID_ATTR_LOCK_TIMEOUT:
        /* msec -> sec */
		lock_timeout_value = Z_LVAL_P(val) * 1000;

		if ((cubrid_retval = cci_set_db_parameter(H->conn_handle, CCI_PARAM_LOCK_TIMEOUT, &lock_timeout_value, &error)) < 0) {
			pdo_cubrid_error(dbh, cubrid_retval, &error, NULL);
			return 0;	
		}
...
static char *pdo_cubrid_last_insert_id(pdo_dbh_t *dbh, const char *name, unsigned int *len TSRMLS_DC)
{
	...
	if ((cubrid_retval = cci_last_insert_id(H->conn_handle, &last_id, &error)) < 0) {
		pdo_cubrid_error(dbh, cubrid_retval, &error, NULL);
		return NULL;
	}
